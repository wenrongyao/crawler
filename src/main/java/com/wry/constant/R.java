package com.wry.constant;

/**
 * 常量类
 * 
 * @author Administrator
 *
 */
public class R {
	public static String WELCOME = "欢迎使用小视频采集软件\n 该软件功能有左边三个：\n 微博视频all是采集微博所有的小视频\n 微博视频key是根据关键字搜索在微博中采集相关视频\n抖音视频key是根据关键字在抖音app中采集相关的视频";
	// 表名
	public static String TABLENAME = "";

	// 请求的cookie
	public static String COOKIE = "SUB=_2AkMu-q8yf8NxqwJRmP0RxWvqbY1-wwrEieKYpl7pJRMxHRl-yT83ql0OtRCjJxWDhzJsLCPANZqnKvd1TRLpRw..; \r\n"
			+ "SUBP=0033WrSXqPxfM72-Ws9jqgMF55529P9D9WFHjjqrLYE8FBTJqjG.faNA; \r\n"
			+ "SINAGLOBAL=1531170698955.3154.1504059402911; \r\n" + "UOR=,,krcom.cn; \r\n"
			+ "ULV=1513576375693:20:10:2:5642275271271.326.1513576375621:1513510051116; ";

	// 西瓜视频的cookie
	public static String COOKIEXIGUA = "odin_tt=1c0d679468615f21d30d89c8dde15792cb223d081befa20a8c235356f8c7bba98b0523f62174e22ace2314c875650f28\r\n"
			+ "	alert_coverage=80\r\n" + "	qh[360]=1\r\n"
			+ "	UM_distinctid=16160cb18000-04a08f668-5137027b-38400-16160cb18056a\r\n" + "	install_id=25377382466\r\n"
			+ "	ttreq=1$2036ff535da35537f55a2599f42dd4cb5b803800\r\n" + "";
	// 主题链接
	public static String TITLE = "title_db";
	// 娱乐
	public static String VFUN = "vfun_db";
	// 游戏
	public static String GAME = "game_db";
	// 舞蹈
	public static String DANCE = "dance_db";
	// 音乐
	public static String MUSIC = "music_db";
	// 电影
	public static String MOVIE = "movie_db";
	// 科技
	public static String TECH = "tech_db";
	// 涨姿势
	public static String DISCOVERY = "discovery_db";
	// 爱生活
	public static String LIFESTYLE = "lifestyle_db";
	// 明星综艺
	public static String SHOW = "show_db";
	// 微天下
	public static String WORLD = "world_db";
	// 体育健身
	public static String SPORTS = "sports_db";
	// 萌宠萌娃
	public static String MOE = "moe_db";
	// 爬取过的url
	public static String VISITED = "visited_db";
	// 微信小视频单次获取条数
	public static Integer VIDEO_NUMBER = 25;
	// 单页一级评论总数
	public static Integer DISCUSS_NUMBER = 15;
	// berkeledb数据库的存放位置
	public static String BERKELEYDB_LOCATION = "//Crawler//berkeleyDB";
	// js的url
	public static String JSURL = "//Crawler//code.js";
	// 当前路径
	public static String CURRENTURL = "D:\\项目\\毕设\\VideoC64";
	// 采集总条数
	public static Integer COLLECTIONCOUNT = 0;
	// 定时任务的时间间隔
	public static Integer TIMETASK = 0;
	// 
}
